package com.jjx.customer.platform.eval.service.impl;
...
import com.jjx.customer.platform.eval.runner.EvalRunner;
...
public class EvalServiceImpl implements EvalService {
    ...
    public void addItems(Long datasetId, List<EvalItemRequest> items) {
        ...
        for (EvalItemRequest req : items) {
            ...
            EvalItemEntity evalItemEntity = new EvalItemEntity();
            evalItemEntity.setDatasetId(datasetId);
            evalItemEntity.setQuestion(req.question());
            evalItemEntity.setExpectedDocIds(req.expectedDocIds() == null ? null : toJson(req.expectedDocIds()));
            evalItemEntity.setExpectedAnswer(req.expectedAnswer());
            evalItemEntity.setCategory(req.category());
            evalItemEntity.setItemKey(req.itemKey());
            evalItemEntity.setSource(StringUtils.hasText(req.source()) ? req.source() : "builtin");
            evalItemEntity.setEnabled(1);
            entities.add(evalItemEntity);
        }
    ...
    public Long triggerRun(EvalRunOptions options) {
        ...
        // 合并检索参数 + paradigm：options.paradigm 优先，否则 paramsOverride 自带，否则 knowledge
        ...
        if (options.paradigm() != null && !options.paradigm().isBlank()) {
            ...
            paradigm = baseParams != null ? baseParams.effectiveParadigm() : "knowledge";
        }
     ...
     */
    @Override
    public PageResult<EvalMetricEntity> getMetrics(Long runId, int page, int size, String metric, Double maxScore) {
        page = Math.max(page, 1);
        size = Math.min(Math.max(size, 1), 200);
        // 低分筛选在库内完成：只留主批次（attempt=0/NULL，与前端透视主行口径一致）该指标 score < 阈值的 item
        boolean badcase = StringUtils.hasText(metric) && maxScore != null;
        com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<EvalMetricEntity> totalWrapper =
                new com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<EvalMetricEntity>()
                        .select("COUNT(DISTINCT item_id) AS cnt")
                        .eq("run_id", runId);
        LambdaQueryWrapper<EvalMetricEntity> idWrapper = new LambdaQueryWrapper<EvalMetricEntity>()
                .select(EvalMetricEntity::getItemId)
                .eq(EvalMetricEntity::getRunId, runId);
        if (badcase) {
            totalWrapper.eq("metric_name", metric).lt("score", maxScore)
                    .and(q -> q.eq("attempt", 0).or().isNull("attempt"));
            idWrapper.eq(EvalMetricEntity::getMetricName, metric)
                    .lt(EvalMetricEntity::getScore, java.math.BigDecimal.valueOf(maxScore))
                    .and(q -> q.eq(EvalMetricEntity::getAttempt, 0).or().isNull(EvalMetricEntity::getAttempt));
        }
        // total = DISTINCT item 数（COUNT(DISTINCT)，不能用 selectCount+groupBy——多行会报错）
        List<java.util.Map<String, Object>> totalRows = metricMapper.selectMaps(totalWrapper);
        ...
        List<Long> pageItemIds = metricMapper.selectList(idWrapper
    ...
    public Long retryRun(Long runId) {
        ...
        run.setParadigm(orig.getParadigm() != null ? orig.getParadigm() : "knowledge");
