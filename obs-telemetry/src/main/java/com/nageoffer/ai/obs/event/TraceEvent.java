 ...
 *
 * <p><b>所属维度</b>：②event（数据模型）。Source 产它，Processor 链加工它，Exporter 链落它。</p>
 *
 * <p><b>职责</b>：承载一次观测事件的全部信息。{@link #data} 是核心载荷——进入 pipeline 时可能是原始对象，
 * 经 Processor 链逐级加工（摘要→截断→[过滤/转换]），到 Exporter 时已是可直写的形态。</p>
 *
 * <p><b>生命周期</b>：事件在 pipeline 中串行流转；任一 Processor 返回 null 即被过滤，不再到 Exporter。
 * Processor 可原地修改 data（{@link #setData}）实现丰富/转换。</p>
 */
@Data
public class TraceEvent {
    ...
    public TraceEvent(EventType type, String name, String spanId, Object data) {
        this.type = type;
        this.name = name;
        this.spanId = spanId;
        this.data = data;
