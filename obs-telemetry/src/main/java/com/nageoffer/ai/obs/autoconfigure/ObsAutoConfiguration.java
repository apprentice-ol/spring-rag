 ...
 *
 * <p>核心 bean：{@link ObsTemplate}（门面）+ {@link ObservationPipeline}（事件处理与落地）。ObservationPipeline 由
...
@ComponentScan(basePackages = "com.nageoffer.ai.obs",
        excludeFilters = @ComponentScan.Filter(type = FilterType.REGEX,
                pattern = "com\\.nageoffer\\.ai\\.obs\\.autoconfigure\\..*"))
public class ObsAutoConfiguration {
     ...
     * @return ObsTemplate 门面
     */
    @Bean
    @ConditionalOnMissingBean
    public ObsTemplate obsTemplate(ObservationRegistry registry, OpenTelemetry openTelemetry,
                                   ObservationPipeline pipeline, ObsProperties properties) {
        return new ObsTemplate(registry, openTelemetry, pipeline, properties.getTracerName());
    }
