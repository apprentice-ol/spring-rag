 ...
 *
 * <p><b>所属维度</b>：转（{@link TraceProcessor} 内置实现，@Order(20)）。</p>
 *
...
@Component
@Order(20)
public class SpanIoLimitProcessor implements TraceProcessor {

    @Override
    public TraceEvent process(TraceEvent event) {
        Object data = event.getData();
        if (!(data instanceof CharSequence cs)) {
            return event;  // 非字符串（已摘要的对象），不截断
        }
        String s = cs.toString();
        if (event.getType() == TraceEvent.EventType.TRACE_IO) {
