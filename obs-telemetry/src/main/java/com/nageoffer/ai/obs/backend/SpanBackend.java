 ...
 *
 * <p><b>所属维度</b>：②backend（策略模式）。{@link ObservationBackend} 与 {@link OtelRootBackend} 是两个具体策略。</p>
 *
 * <p><b>职责</b>：在"已 open"的 span 上提供"写属性 / 记异常 / 关 scope / 结束 span"。open 动作在各实现的静态工厂 {@code createAndOpen} 完成。</p>
 *
 * <p><b>协作</b>：被 {@code event.TraceHandle} 持有（每 handle 一个实例）；作为 {@link SpanWriter} 传给 sink 链。</p>
 *
 * <p><b>不做什么</b>：不发结构化日志、不管 MDC ambient 标签、不感知业务。</p>
 */
public interface SpanBackend extends SpanWriter {
