 ...
 *
 * <p><b>职责</b>：加工或处置一个 {@link TraceEvent}。返回加工后的事件（可改 {@code setData} 原地转换），
 * 或返回 <b>null</b> 将其过滤（不再流向 Exporter）。</p>
 *
 * <p><b>扩展点</b>（框架的核心扩展面）：实现本接口 + {@code @Component} + {@code @Order} 即自动进
 * {@link TracePipeline} 的 processor 链。内置：摘要（Summarize）、截断（SpanIoLimit）；可扩展：敏感过滤、
 * 采样、语义映射（如 rag.trace.* → langfuse.*）等。processor 必须无状态（pipeline 全局共享，多线程调用）。</p>
 *
 * <p><b>不做什么</b>：不写 span / 不发日志（那是 Exporter 的职责）；不开/关 span。</p>
 */
public interface TraceProcessor {
     ...
     *
     * @param event 流入的事件
     * @return 加工后的事件（原对象或新对象）；null 表示过滤掉（不流向 Exporter）
     */
    TraceEvent process(TraceEvent event);
}
