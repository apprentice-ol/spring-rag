 ...
 *
 * <p><b>协作</b>：由 {@code Telemetry.openTrace} / {@code trigger.TraceStepAspect}（{@code kind=ROOT}）经 {@link #createAndOpen} 创建。tracer 名由 {@code TelemetryProperties.tracerName} 传入（默认 "obs"）。</p>
 *
 * <p><b>后端归属（后端无关）</b>：操作 OTel span，经 Collector 分发到多后端。</p>
 */
public final class OtelRootBackend implements SpanBackend {

    private final Span span;
    private final String spanId;
    private final io.opentelemetry.context.Scope scope;

    private OtelRootBackend(Span span, String spanId, io.opentelemetry.context.Scope scope) {
        this.span = span;
        this.spanId = spanId;
        this.scope = scope;
    }
    ...
    public static OtelRootBackend createAndOpen(String name, OpenTelemetry otel, String tracerName) {
        ...
        return new OtelRootBackend(span, spanId, scope);
    }
