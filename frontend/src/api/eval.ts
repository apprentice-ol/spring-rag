 ...
 * 低分筛选（metric + maxScore）由后端在库内完成：只留主批次该指标 score < maxScore 的 item。
 */
export async function getMetrics(
    runId: number,
    page = 1,
    size = 20,
    filter?: { metric?: string; maxScore?: number },
): Promise<MetricsPage> {
    const {data} = await http.get<MetricsPage>(`/eval/runs/${runId}/metrics`, {
        params: {page, size, metric: filter?.metric, maxScore: filter?.maxScore},
    })
    return data
}
...
let ooTraceTpl = ''
getObserveLinks().then((l) => {
    ooTraceTpl = l.traceDetailUrlTemplate || ''
}).catch(() => {
})
