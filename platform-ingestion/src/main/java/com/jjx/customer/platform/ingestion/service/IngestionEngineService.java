...
import com.jjx.customer.platform.ingestion.engine.IngestionEngine;
import com.jjx.customer.platform.ingestion.engine.NodeLog;
import com.jjx.customer.platform.ingestion.engine.PipelineDefinition;
...
import com.jjx.customer.platform.ingestion.mapper.IngestionTaskNodeMapper;
...
public class IngestionEngineService {
    ...
    private final DocumentVersionStamp documentVersionStamp;
    ...
    public IngestionResult executeTask(String pipelineId, DocumentSource source, byte[] bytes,
        ...
        String effectiveDocId = StringUtils.hasText(docId) ? docId : taskId;

        long startTime = System.currentTimeMillis();
        log.info("[INGEST] 开始: pipelineId={}, taskId={}, docId={}, mimeType={}, plainText={}", pipelineId, taskId, effectiveDocId, mimeType, plainText);
        ...
        // 缓存失效钩子：文档落库即 bump 版本号（检索/答案缓存的 key 含 docver，自然换 key；
        // FAILED 状态也 bump——保守失效可接受，宁可多查不可脏读）。自吞异常，不影响入库链路。
        documentVersionStamp.bump(collectionId);
        ...
        log.info("[INGEST] 完成: taskId={}, docId={}, chunks={}, status={}, 总耗时 {}ms",
                taskId, effectiveDocId, chunkCount, status, System.currentTimeMillis() - startTime);
