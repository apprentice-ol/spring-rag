...
import com.jjx.customer.platform.ingestion.engine.chunk.strategy.ChunkingMode;
import com.jjx.customer.platform.ingestion.engine.chunk.strategy.ChunkingStrategyFactory;
import com.jjx.customer.platform.ingestion.engine.chunk.strategy.ChunkingOptions;

import cn.hutool.core.util.IdUtil;
import com.jjx.customer.platform.ingestion.engine.chunk.blockaware.BlockAwareChunkerDispatcher;
import com.jjx.customer.platform.ingestion.engine.chunk.blockaware.BlockChunkConfig;
import com.jjx.customer.platform.ingestion.engine.parser.BlockTextRenderer;
...
public class StructuredChunkingService {
    ...
    private static String languageSample(List<Block> blocks, String fallbackText) {
        ...
        if (blocks != null && !blocks.isEmpty()) {
            ...
            for (Block b : blocks) {
                if (b instanceof com.jjx.customer.platform.ingestion.engine.parser.model.ParagraphBlock p
                        && p.text() != null) {
    ...
    private static String blockTypeName(Block b) {
        if (b instanceof com.jjx.customer.platform.ingestion.engine.parser.model.HeadingBlock) return "HEADING";
        if (b instanceof com.jjx.customer.platform.ingestion.engine.parser.model.ParagraphBlock) return "PARAGRAPH";
        if (b instanceof com.jjx.customer.platform.ingestion.engine.parser.model.TableBlock) return "TABLE";
        if (b instanceof com.jjx.customer.platform.ingestion.engine.parser.model.ImageBlock) return "IMAGE";
        if (b instanceof com.jjx.customer.platform.ingestion.engine.parser.model.CodeBlock) return "CODE";
        if (b instanceof com.jjx.customer.platform.ingestion.engine.parser.model.ListBlock) return "LIST";
        return "PARAGRAPH";
    }
