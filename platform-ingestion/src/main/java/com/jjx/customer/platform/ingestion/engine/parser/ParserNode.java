...
package com.jjx.customer.platform.ingestion.engine.parser;
...
import com.jjx.customer.platform.ingestion.engine.parser.reader.MimeTypeDetector;
...
public class ParserNode implements IngestionNode {
    ...
    public NodeResult execute(IngestionContext context, NodeConfig config) {
        if (context.getRawBytes() == null || context.getRawBytes().length == 0) {
            log.info("解析器缺少{}原始字节", context.getTaskId());
        ...
        if (parser == null) {
            log.info("未找到 MIME [{}] 对应的解析器,fileName={}", mimeType, fileName);
        ...
        // 把 unpackMode 注入 options（MinerU 解析路线：markdown 默认 / content_list 结构化表格）
        if (StringUtils.hasText(settings.getUnpackMode()) && !options.containsKey("unpackMode")) {
            options.put("unpackMode", settings.getUnpackMode());
        }
