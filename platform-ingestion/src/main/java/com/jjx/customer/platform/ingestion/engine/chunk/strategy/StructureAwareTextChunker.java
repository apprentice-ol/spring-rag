...
package com.jjx.customer.platform.ingestion.engine.chunk.strategy;
...
public class StructureAwareTextChunker implements ChunkingStrategy {
    ...
    public StructureAwareTextChunker(BoundaryAwareSplitter boundaryAwareSplitter) {
    ...
    public List<VectorChunk> chunk(String text, ChunkingOptions config) {
        ...
        // 3) 物化为 chunk：超长 range（单个 PARA 块超 max）按边界感知二次切，不再原样吐整个超长块；
        //    可选 overlap：复制上一 chunk 原文结尾到下一 chunk 开头
        return materialize(text, ranges, effectiveOverlap, opts);
    }
    ...
    // ----------- 3) 物化为 Chunk：超长 range 按边界感知二次切，可选 overlap -----------
    private List<VectorChunk> materialize(String text, List<int[]> ranges, int overlap, TextBoundaryOptions opts) {
        ...
        for (int[] range : ranges) {
            ...
            // 超长 range（单个块/打包块超过 max）：按边界降级二次切，不再原样输出整个超长块
            List<String> pieces = body.length() > opts.maxChars()
                    ? boundaryAwareSplitter.split(body, opts.minChars(), opts.targetChars(), opts.maxChars(), 0)
                    : List.of(body);

            for (String piece : pieces) {
                String content = (overlap > 0 && prevTail != null && !prevTail.isEmpty())
                        ? prevTail + piece
                        : piece;
            ...
            // 下一 chunk 的 overlap 尾部取自本 range 的原文结尾（不含已拼接的 prevTail，避免尾部滚动膨胀）
            if (overlap > 0) {
                prevTail = tailByChars(body, overlap);
            }
        }
        return out;
    }
