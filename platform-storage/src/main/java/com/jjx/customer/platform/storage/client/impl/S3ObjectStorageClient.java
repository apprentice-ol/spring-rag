package com.jjx.customer.platform.storage.client.impl;
...
public class S3ObjectStorageClient implements ObjectStorageClient {
    ...
    @PostConstruct
    public void ensureAssetBucketPublic() {
        try {
            if (!bucketExists(assetBucket)) {
                createBucket(assetBucket);
                log.info("[S3ObjectStorageClient] 创建资产桶: {}", assetBucket);
            }
            setBucketPublicRead(assetBucket);
            log.info("[S3ObjectStorageClient] 资产桶已设为公开读: {}", assetBucket);
        } catch (Exception e) {
            log.warn("[S3ObjectStorageClient] 初始化资产桶失败: {}", e.getMessage());
        }
    }
