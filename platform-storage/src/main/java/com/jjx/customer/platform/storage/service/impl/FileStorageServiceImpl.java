...
package com.jjx.customer.platform.storage.service.impl;

import cn.hutool.core.lang.Assert;
import com.jjx.customer.platform.common.exception.ServiceException;
import com.jjx.customer.platform.config.properties.RagStorageProperties;
import com.jjx.customer.platform.storage.client.ObjectStorageClient;
import com.jjx.customer.platform.storage.domian.dto.StoredFileDTO;
import com.jjx.customer.platform.storage.service.FileStorageService;
import com.jjx.customer.platform.common.util.FileTypeDetector;
 ...
 * 底层裸操作委托给注入的 {@link S3ObjectStorageClient}（S3 或 OSS，由 {@code rag.storage.type} 决定）
 * <p>
...
public class FileStorageServiceImpl implements FileStorageService {
    ...
    public FileStorageServiceImpl(ObjectStorageClient objectStorageClient,
    ...
    /**
     * 启动时自动创建需要的桶，幂等安全，已存在则跳过
     */
    @PostConstruct
    public void initBuckets() {
        log.info("初始化 S3 桶: kb={}, asset={}", kbBucket, assetBucket);
        safeCreateBucket(kbBucket);
        safeCreateBucket(assetBucket);
        ensureAssetBucketPublic();
    }

    private void safeCreateBucket(String bucket) {
        try {
            if (objectStorageClient.bucketExists(bucket)) {
                log.debug("桶已存在: {}", bucket);
                return;
            }
            objectStorageClient.createBucket(bucket);
            log.info("桶创建成功: {}", bucket);
        } catch (Exception e) {
            log.warn("桶初始化失败（可能非 S3 存储或权限不足），后续上传将自动重试: bucket={}, error={}",
                    bucket, e.getMessage());
        }
    }

    private void ensureAssetBucketPublic() {
        try {
            objectStorageClient.setBucketPublicRead(assetBucket);
            log.info("资产桶设为公共读: {}", assetBucket);
        } catch (Exception e) {
            log.warn("资产桶设公共读失败（图片链接可能无法直接预览）: bucket={}, error={}",
                    assetBucket, e.getMessage());
        }
        // kbBucket 也设为公共读，供前端直连预览文档源文件
        try {
            objectStorageClient.setBucketPublicRead(kbBucket);
            log.info("文档桶设为公共读: {}", kbBucket);
        } catch (Exception e) {
            log.warn("文档桶设公共读失败: bucket={}, error={}", kbBucket, e.getMessage());
        }
    }
    ...
    public StoredFileDTO uploadAsset(byte[] content, String originalFilename, String contentType) {
        ...
        // 用 reliablePut（直连 SDK）而非 streamPut（预签名 URL），
        // 避免 RustFS 对 AWS SigV4 预签名 URL 兼容不足导致 HTTP 401
        objectStorageClient.reliablePut(assetBucket, key, new ByteArrayInputStream(content), content.length, detected);
        return buildStoredFileDTO(key, originalFilename, detected, content.length);
    }
    ...
    @Override
    public String getKbPublicUrl(String key) {
        Assert.notBlank(key, "对象 key 不能为空");
        return objectStorageClient.buildPublicUrl(kbBucket, key);
    }
