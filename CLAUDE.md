...
- **2026-09-10 重构**：WorkflowEngine 引擎化（决策 4）+ 编排层三组件拆分 + Redis 五层缓存（决策 9）。
...
5. **缓存层**：~~embedding 缓存 + 语义查询缓存~~ **已全部落地**（2026-09-10，见决策 9：五层 Redis 缓存 + Sentinel-lite 频率准入/热度 TTL + 熔断联动动态限流 + pgvector 语义答案缓存）。反馈类失效（点踩回流）明确不做；可演进项：双窗口趋势判定（升降温自适应准入）、断路器恢复 AIMD 渐进放量。
