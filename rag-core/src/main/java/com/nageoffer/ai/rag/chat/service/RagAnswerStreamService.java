 ...
 * bean 方法，用 {@link TraceStep} 注解自动埋点（开 span + 完整回答捕获 + 流终止 finish）。
 *
 * <p>抽独立 bean 的原因：流式方法的 span 生命周期要覆盖整个 Flux 订阅期，只能靠 {@link TraceStepAspect}
...
public class RagAnswerStreamService {
    ...
    /** 问候/闲聊流式回答（不检索）。 */
    @TraceStep(value = "rag.chitchat", captureOutput = true)
    ...
    @TraceStep(value = "rag.answer", captureOutput = true)
