...
/**
 * 条件评估器（搬自原 ragent，改 package）。
 *
 * <p>支持：布尔 / SpEL 文本 / all / any / not / 字段规则（eq/ne/in/contains/regex/gt/gte/lt/lte/exists/not_exists）。
 */
@Component
public class ConditionEvaluator {
    ...
    private Object readField(IngestionContext context, String path) {
        try {
            return new BeanWrapperImpl(context).getPropertyValue(path);
        } catch (Exception e) {
            return null;
        }
    }
